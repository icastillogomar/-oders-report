...
func (h *OrdersHandler) handleGetErrorCodes(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.service.GetErrorCodes(company, productType, fulfillmentType, startDate, endDate)
	if err != nil {
		...
		w.Header().Set("Content-Type", "application/json")
		if errors.Is(err, services.ErrInvalidDateFormat) || errors.Is(err, services.ErrInvalidFulfillmentType) {
			w.WriteHeader(http.StatusBadRequest)
			_ = json.NewEncoder(w).Encode(map[string]interface{}{
				"error": err.Error(),
			})
